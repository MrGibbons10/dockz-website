...
<body style="margin:0;padding:0;background:#0a0a0d;">
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#0a0a0d;padding:32px 12px;">
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;">
        <tr><td style="padding:0 8px 18px;font-family:'Space Grotesk',Arial,sans-serif;font-size:24px;font-weight:700;color:#f4f4f6;letter-spacing:.5px;">
          Dock<span style="color:${ACCENT};">z</span>
        </td></tr>
        <tr><td style="background:#101014;border:1px solid rgba(255,255,255,.09);border-radius:20px;padding:36px 32px;font-family:Inter,Arial,sans-serif;color:#f4f4f6;">
          ${inner}
        </td></tr>
        <tr><td style="padding:20px 8px;font-family:Inter,Arial,sans-serif;font-size:12px;line-height:1.6;color:#6b6b76;">
...
const btn = (href, label) =>
  `<a href="${href}" style="display:inline-block;background:${ACCENT};color:#062a33;font-family:Inter,Arial,sans-serif;font-weight:700;font-size:15px;text-decoration:none;padding:13px 26px;border-radius:12px;">${label}</a>`;
...
export function welcomeEmail({ siteUrl, code = 'WELCOME10' }) {
  return {
    ...
    html: shell(`
      <h1 style="margin:0 0 14px;font-family:'Space Grotesk',Arial,sans-serif;font-size:26px;line-height:1.25;color:#f4f4f6;">Welcome to Dockz.</h1>
      <p style="margin:0 0 18px;font-size:15px;line-height:1.65;color:#a3a3ad;">
        Thanks for signing up! Here's your welcome discount — <b style="color:#f4f4f6;">10% off anything</b> in the shop.
        Use it at checkout whenever you're ready.
      </p>
      <div style="background:#16161c;border:1px dashed rgba(99,230,255,.5);border-radius:14px;padding:20px;text-align:center;margin:0 0 22px;">
        <div style="font-size:12px;letter-spacing:2px;color:#6b6b76;text-transform:uppercase;margin-bottom:6px;">Your code</div>
      ...
      <p style="margin:0 0 24px;font-size:15px;line-height:1.65;color:#a3a3ad;">
...
export function orderConfirmationEmail({ orderId, items, totalPaid, shipTo, name, hasPreorder, siteUrl }) {
  const itemRows = items.map(i => `
    <tr>
      <td style="padding:10px 0;font-size:14px;color:#f4f4f6;border-bottom:1px solid rgba(255,255,255,.07);">${i.qty}× ${i.description}</td>
      <td style="padding:10px 0;font-size:14px;color:#f4f4f6;border-bottom:1px solid rgba(255,255,255,.07);text-align:right;white-space:nowrap;">${i.lineTotal}</td>
    </tr>`).join('');
  ...
  return {
    ...
    html: shell(`
      <h1 style="margin:0 0 14px;font-family:'Space Grotesk',Arial,sans-serif;font-size:26px;line-height:1.25;color:#f4f4f6;">You're all docked in${name ? ', ' + name.split(' ')[0] : ''}.</h1>
      <p style="margin:0 0 22px;font-size:15px;line-height:1.65;color:#a3a3ad;">
        ...
        ${hasPreorder ? '<br><br><b style="color:#f4f4f6;">Heads up:</b> your order includes a pre-order item — we’ll keep you posted on timing.' : ''}
      </p>
      <div style="background:#16161c;border-radius:14px;padding:18px 20px;margin:0 0 22px;">
        <div style="font-size:12px;letter-spacing:2px;color:#6b6b76;text-transform:uppercase;margin-bottom:4px;">Order reference</div>
        <div style="font-size:14px;color:#f4f4f6;word-break:break-all;">${orderId}</div>
      </div>
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 6px;">
        ${itemRows}
        <tr>
          <td style="padding:14px 0 0;font-size:15px;font-weight:700;color:#f4f4f6;">Total paid (incl. shipping)</td>
          <td style="padding:14px 0 0;font-size:15px;font-weight:700;color:${ACCENT};text-align:right;">${totalPaid}</td>
        </tr>
      </table>
      <p style="margin:20px 0 24px;font-size:14px;line-height:1.6;color:#a3a3ad;">
        <b style="color:#f4f4f6;">Shipping to:</b><br>${addr}
      </p>
...
export function orderAlertEmail({ orderId, items, totalPaid, shipTo, name, email, phone, hasPreorder }) {
  ...
  return {
    ...
    html: shell(`
      <h1 style="margin:0 0 14px;font-family:'Space Grotesk',Arial,sans-serif;font-size:22px;color:#f4f4f6;">New order in. Time to print. 🖨️</h1>
      <ul style="margin:0 0 16px;padding-left:18px;font-size:14px;line-height:1.7;color:#f4f4f6;">${lines}</ul>
      <p style="margin:0;font-size:14px;line-height:1.8;color:#a3a3ad;">
        <b style="color:#f4f4f6;">Total:</b> ${totalPaid}<br>
        <b style="color:#f4f4f6;">Customer:</b> ${name || '—'} · ${email || '—'} · ${phone || '—'}<br>
        <b style="color:#f4f4f6;">Ship to:</b> ${addr}<br>
        <b style="color:#f4f4f6;">Stripe ref:</b> ${orderId}
      </p>
    `),
  };
}
...
export function subscriberAlertEmail({ email }) {
  return {
    ...
    html: shell(`
      <p style="margin:0;font-size:15px;line-height:1.7;color:#f4f4f6;">
        <b>${email}</b> just signed up for the 10% welcome offer.<br>
        <span style="color:#a3a3ad;font-size:13px;">Tip: search your inbox for "New Dockz subscriber" to see your full list.</span>
      </p>
    `),
  };
}
